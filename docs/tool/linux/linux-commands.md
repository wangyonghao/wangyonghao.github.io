...
- `lsof -i -nP | grep 8080` 或 `netstat -tulnp ｜ grep  8080`：查询哪个程序占用了端口



**查看系统信息**
 显示机器的处理器架构(1)
`uname -m`  或`arch` 显示机器的处理器架构(2)
uname -r 显示正在使用的内核版本
...
kill -9 sid 使用进程号结束进程
killall -9 name 使用进程名结束进程

**终端**

chsh -s /bin/zsh 切换到zsh终端，Zsh有强大的自动补全参数和自定义配置功能
chsh -s /bin/bash 切换到bash终端
